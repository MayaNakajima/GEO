...
作成日：2026年7月9日
更新日：2026年7月28日（**v8：ダッシュボードの分析改善（過去回比較の言語化・Set比較/崖の分析タブ・用語補足）／ローカルGit運用へ移行**）
前版：2026年7月27日（v7：質問セット選択（Set1/Set2/両方）・特異度ティア・CSV列追加）

---

## 0. v8 での主な変更点（2026-07-28）

**ダッシュボード（`src/dashboard.py`）の「分析の見せ方」を改善**。数値・グラフは揃っていたが「どこを見れば何が分かるか」が読み手に伝わりにくかったため、以下を追加した。**集計側（`analytics.py`／`reporter.py`）は無改修**（必要な値 `deltas`・`insights`(tier_curve含む)・`tier_stats`・`set_stats` は既に `trend.json`／`timing_*.json` に出力済み）。追加はすべて自己完結HTML＋Chart.js への追記で完結する。

- **2-1 過去回との比較を言語化（時系列トレンドタブ）**
  - 画面最上部に**1文サマリーバナー**を自動生成。`trend.json` の `deltas`・`latest`・`tier_series` から「最新は全体●%（前回比 ▲+●pt 改善／初回比 …）。ヒットは指名質問（D1）に集中…」を組み立て、良化=緑・悪化=赤・横ばい=グレーで色分け。
  - 各タブ冒頭に**「この画面の見方」折りたたみ注記**を追加（見る順序を明示）。
  - KPIカードの矢印に**「改善／低下／横ばい」の言葉を併記**（例 `▲ +2.5pt 改善`）。
  - 前回比変動（movers）表の上に**見方1行**（プラス＝改善・マイナス＝低下）を追加。

- **2-2 Set比較・崖の分析タブを新設（3つ目のタブ）**
  - **特異度カーブ図**（横軸 D1→D4／縦軸 出現率）を主役に配置。Set2の `tier_stats` を棒＋折れ線で描き、**崖ティア（出現率<10%に落ちる最初のティア）を赤で強調**。Set1全体平均を**「D4相当の基準線」として点線で重畳**。
  - 図の直下に**平易な解釈文**（崖の位置・意味・埋めるべきギャップ）を大きめに表示。
  - 崖の位置に応じて出し分ける**次アクション**（箇条書き）。
  - **Set1×Set2 サマリー表**（全体出現率／出現の中心／読み取り＋各セルの役割説明）。

- **2-3 凡例・単位・用語の補足**
  - 凡例名を平易化（`D2`→「D2 業種・実績」、`set2`→「Set2 診断用60問」等）。
  - 見出し横に**用語ツールチップ（❓ホバー）**：SD（標準偏差）／安定性スコア／特異度（D1〜D4）の一言説明。
  - 空データ時の注記文言を整備（「Set2を実行すると表示されます」等）。

- **リポジトリ運用への移行**：本プロジェクトをローカルGitリポジトリ（`github.com/MayaNakajima/GEO`）で管理する運用へ移行。機密（`.env`）・生成データ（`data/` の results/logs/reports・dashboard.html）・キャッシュ・バックアップは `.gitignore` で除外（§15）。

> 崖の判定・数値・カーブ形状は集計結果に連動。現行の実測（2026-07-27 17:05・Set1+Set2・2回）では **崖は D1→D2**（D1 指名 20%／D2 業種・実績 0%以下）で、埋めるべきGEOギャップは D2（業種特化・実績）という結論がダッシュボード上に文章・図で明示される。

---

## 0-1. v7 での主な変更点（2026-07-27）
...
> **前提（測定対象）**：現在の稼働モデルはClaude単体＝**非グラウンディング**。測っているのは「AIの学習記憶に当社があるか」。検索引用の測定はPerplexity等のキー許可後（将来）。
...
- モデル別・事業別、**質問セット別（`set_stats`）・特異度ティア別（`tier_stats`）** の平均±SD
→ `data/reports/timing_<timing_id>.json`
- ※ M=1（1回のみ）ではブレは測定できません。ブレを見るには2回以上の実行が必要です。

### Tier3：複数タイミング（cross-timing・トレンド／比較／示唆）
タイミングをまたいだ推移を分析。
- **全体出現率の時系列**（平均±SDの帯グラフ）、モデル別・事業別・**特異度ティア別／質問セット別**の推移
- **前回比／初回比**の差分、事業ドメイン別の変動（movers）
- **施策への示唆（自動生成）**：コンテンツ空白（低出現ドメイン）、あと一歩（ブレている質問）、低下／改善ドメイン、**特異度の崖（`tier_curve`）**
→ `data/reports/trend.json`

### 出力形式（HTMLダッシュボード） ★v8で強化
`data/dashboard.html`（グラフ付き・自己完結。GUIの「ダッシュボードを開く」または直接ブラウザで開く）。**3タブ構成**。

1. **時系列トレンド**：★v8で冒頭に**1文サマリーバナー**（前回比・初回比を「改善／低下／横ばい」の言葉と色で明示）、**「この画面の見方」**折りたたみ注記、KPI矢印への言葉併記、movers表の見方注記を追加。
2. **タイミング詳細（ブレ）**：同一タイミングM回のブレ・安定性。★v8で見方注記・用語ツールチップを追加。
3. **Set比較・崖の分析（★v8新設）**：Set2の**特異度カーブ（D1→D4）**を主役に、崖ティアを赤で強調、Set1平均を点線基準で重畳。崖の位置バッジ・**平易な解釈文**・**崖に応じた次アクション**・**Set1×Set2サマリー表**を表示。

その他の出力：**JSON**（上記各レポート）、**rowデータCSVエクスポート**（回答全文・検出情報を含む、UTF-8 BOM）、**Teams通知（任意）**（`.env` に `TEAMS_WEBHOOK_URL` 設定時）。

> ★v8の追加はすべて `dashboard.py`（自己完結HTML＋Chart.js CDN）への追記で完結。崖判定は「出現率<10%に落ちる最初のティア」を採用（`analytics._add_tier_insights` と同一の考え方）。追加ライブラリなし（崖は棒の赤塗り＋図中テキストで表現）。
...
| dashboard.html | data/ | HTMLダッシュボード（★v8：3タブ・サマリーバナー・Set比較タブ） |
| run_log.txt | data/ | 実行ログ（CLI自動実行時のエラー確認用） |

- `run_id` = `<timing_id>_r<回番号>`（例：`20260716_090000_r1`）、`timing_id` = 実行時点（`YYYYMMDD_HHMMSS`）。
- **★v8：`data/` 配下の生成物は Git 追跡対象外**（`.gitignore`）。フォルダ構造は `.gitkeep` で保持し、実行時に再生成される。
...
├── .env                      APIキー等（機密・Git非格納 ★v8）※各自ローカル
...
│   ├── analytics.py          ★v6 Tier2/Tier3 集計・ブレ/トレンド/示唆（★v8で無改修）
│   ├── dashboard.py          ★v8 HTMLダッシュボード生成（3タブ・サマリー・Set比較）
...
**セキュリティ注意**：`.env` の共有キーはフォルダ閲覧者全員が見られる。アクセス権を必要メンバーに限定し、用途専用キーを使用、露出した旧キーは失効・再発行する。

---

## 15. リポジトリ運用（Git）★v8

**本プロジェクトはローカルGitリポジトリで管理する運用へ移行**（リモート：`github.com/MayaNakajima/GEO`）。作業環境はローカルGit環境（`Documents/GitHub/GEO`）を正とする。

### 15-1. Gitに含めるもの／含めないもの

| 区分 | 対象 | 扱い |
|---|---|---|
| 含める | `src/*.py`・`config/*.json`・`webapp/`・`*.bat`・`run.py`・`requirements.txt`・`.env.example`・各設計/仕様/運用ドキュメント | 追跡 |
| 含めない | `.env`（機密）／`data/` 配下の生成物（results・logs・reports・dashboard.html）／`__pycache__`・`*.pyc`／`*.bak`／`*.zip`・`*.xlsx`（予算表等） | `.gitignore` で除外 |

- `data/` のフォルダ構造は `.gitkeep` で保持（中身は追跡しない）。クローン後は実行時に再生成される。
- **機密キーは絶対にコミットしない。** APIキーは各自ローカルの `.env` に設定（`.env.example` を複製して記入）。

### 15-2. 基本フロー
```bash
git clone https://github.com/MayaNakajima/GEO.git
cd GEO/monitoring
cp .env.example .env        # 各自のAPIキーを記入（Gitには載らない）
pip install -r requirements.txt
python run.py               # または monitoring_gui.bat
```
- 変更をコミット：`git add -A && git commit -m "説明"` → `git push`。
- 誤って `.env` を追跡しないよう、`git status` で `.env` が「Untracked/Ignored」であることを確認する。

---

## 変更履歴
- **v8（2026-07-28）**：ダッシュボード（`dashboard.py`）の分析改善。①過去回比較の言語化（1文サマリーバナー・「この画面の見方」注記・KPI矢印の言葉併記・movers見方注記）、②「Set比較・崖の分析」タブ新設（特異度カーブ＋崖の赤強調＋Set1平均の点線基準＋解釈文＋次アクション＋Set1×Set2サマリー表）、③用語・凡例の補足（凡例名の平易化・SD/安定性/特異度の❓ツールチップ・空データ注記）。集計側（`analytics.py`／`reporter.py`）は無改修。あわせてプロジェクトをローカルGit運用へ移行し `.gitignore` を整備（`.env`・生成データ・キャッシュ・バックアップ・アーカイブを除外）。
